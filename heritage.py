	...
	for word in cmpdict:
		if word[0] in fragment:
			...
			if len(wordleft) == len(wordright) == 0:
				...
				multiplier += word[1] * compare(wordleft, cmpdict[cmpdict.index(word):]) * compare(wordright, cmpdict[cmpdict.index(word):])
			elif len(wordleft) != 0:
				multiplier += word[1] * compare(wordleft, cmpdict[cmpdict.index(word):])
			elif len(wordright) != 0:
				multiplier += word[1] * compare(wordright, cmpdict[cmpdict.index(word):])
